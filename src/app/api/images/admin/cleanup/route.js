// app/api/images/admin/cleanup/route.js
// Native Next.js App Router handler.
// Business logic stays in images.service (cleanupUnusedImages).
...
export async function POST(req) {
  ...
  // Orchestrate (was cleanupUnusedImages)
  try {
    const result = await imagesService.cleanupUnusedImages();
    return new NextResponse(JSON.stringify({ success: true, ...result }), { status: 200, headers: { 'content-type': 'application/json; charset=utf-8' } });
  } catch (err) {
