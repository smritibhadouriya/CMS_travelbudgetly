...
//
// Native Next.js App Router handlers for Offers list/create (Phase-4 step 4).
//
//   Route → parse/auth → service → Prisma
//
// All business logic stays in offer.service (buildOfferPayload / listOffers /
// createOffer); sharp conversion in upload.service. The handler only parses the
// request, enforces auth, wires the services, and shapes the response —
...
export async function GET(req) {
  try {
    ...
    return json({ success: false, message: "Failed to fetch offers" }, 500);
  }
}
...
export async function POST(req) {
  ...
  // 4. Orchestrate (was createOffer) — business logic stays in the service
  try {
    const raw     = parseBody({ body });
    const payload = offerService.buildOfferPayload(raw, files);
    if (!payload.slug) return json({ success: false, message: "Heading or slug is required" }, 400);

    const offer = await offerService.createOffer(payload);
    return json({ success: true, message: "Offer created", data: offer }, 201);
  } catch (err) {
    if (err.code === "P2002") return json({ success: false, message: "An offer with this slug already exists." }, 400);
    console.error("❌ createOffer:", err.message);
    return json({ success: false, message: "Failed to create offer" }, 500);
  }
}
