...
/* ── Helper: build FormData (data JSON + named file fields) ── */
export const buildFormData = (data, files = {}) => {
  const fd = new FormData();
  fd.append("data", JSON.stringify(data));
  Object.entries(files).forEach(([key, file]) => {
    if (Array.isArray(file)) {
      file.forEach((f) => { if (f instanceof File) fd.append(key, f); });
...
export const getFeaturedPackages = (params = {}) => getPackages({ isFeatured: true, isPublished: true, ...params });
...
══════════════════════════════════════ */
export const addComment          = (blogId, data)  => api.post(`/comments/blog/${blogId}`, data);
export const getApprovedComments = (blogId)        => api.get(`/comments/blog/${blogId}/approved`);
...
export const getSeoByPage = (page)         => api.get("/seo", { params: { page } });
...
export const deleteImage     = (filename) => api.delete(`/images/${encodeURIComponent(filename)}`);
